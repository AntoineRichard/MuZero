...
def run_mcts(config,
  ...
  for _ in range(config.num_simulations):
    ...
    #print(parent.hidden_state.shape)
    #print(history.last_action())
    #network_output = recurrent_inference_fn(np.expand_dims(parent.hidden_state,0),
    #                                        np.expand_dims(history.last_action(),0))
...
def expand_node(node, legal_actions,
  ...
  policy_probs = masked_softmax(
      np.squeeze(network_output.policy_logits), mask=legal_actions.astype(np.float64))
