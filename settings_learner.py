...
SAVE_CHECKPOINT_SECS            = flags.DEFINE_integer('save_checkpoint_secs',
                                                    1800,
                                                    'Checkpoint save period in seconds.')
TOTAL_ITERATIONS                = flags.DEFINE_integer('total_iterations',
                                                    int(1e6),
                                                    'Total iterations to train for.')
BATCH_SIZE                      = flags.DEFINE_integer('batch_size',
                                                    64,
                                                    'Batch size for training.')
REPLAY_QUEUE_BLOCK              = flags.DEFINE_integer('replay_queue_block',
                                                    0,
                                                    'Whether actors block when enqueueing.')
RECURRENT_INFERENCE_BATCH_SIZE  = flags.DEFINE_integer('recurrent_inference_batch_size',
                                                    1,
                                                    'Batch size for the recurrent inference.')
INITIAL_INFERENCE_BATCH_SIZE    = flags.DEFINE_integer('initial_inference_batch_size',
                                                    1,
                                                    'Batch size for initial inference.')
NUM_TRAINING_TPUS               = flags.DEFINE_integer('num_training_tpus',
                                                    1,
                                                    'Number of TPUs for training.')
INIT_CHECKPOINT                 = flags.DEFINE_string('init_checkpoint',
                                                    None,
                                                    'Path to the checkpoint used to initialize the agent.')
NUM_ACTORS                      = flags.DEFINE_integer('num_actors',
                                                    10,
                                                    'Number of actors.')
REPLAY_BUFFER_SIZE              = flags.DEFINE_integer('replay_buffer_size',
                                                    1000,
                                                    'Size of the replay buffer.')
REPLAY_QUEUE_SIZE               = flags.DEFINE_integer('replay_queue_size',
                                                    100,
                                                    'Size of the replay queue.')
REPLAY_BUFFER_UPDATE_PRIORITY_AFTER_SAMPLING_VALUE = flags.DEFINE_float('replay_buffer_update_priority_after_sampling_value',
                                                    1e-6,
                                                    ...
                                                    'be done.')
FLUSH_LEARNER_LOG_EVERY_N_S     = flags.DEFINE_integer('flush_learner_log_every_n_s',
                                                    60,
                                                    'Size of the replay buffer (in number of batches stored).')
ENABLE_LEARNER_LOGGING          = flags.DEFINE_integer('enable_learner_logging',
                                                    1,
                                                    'If true (1), logs are written to tensorboard.')
LOG_FREQUENCY                   = flags.DEFINE_integer('log_frequency', 100, 'in number of training steps')

IMPORTANCE_SAMPLING_EXPONENT    = flags.DEFINE_float('importance_sampling_exponent',
                                                    0.0,
...
PRIORITY_SAMPLING_EXPONENT      = flags.DEFINE_float('priority_sampling_exponent',
                                                    0.0,
                                                    ...
                                                    'a wrong value.')
LEARNER_SKIP                    = flags.DEFINE_integer('learner_skip',
                                                    0,
                                                    'How many batches the learner skips.')
EXPORT_AGENT                    = flags.DEFINE_integer('export_agent',
                                                    0,
                                                    'Save the agent in ExportAgent format.')
WEIGHT_DECAY                    = flags.DEFINE_float('weight_decay',
                                                    1e-5,
                                                    'l2 penalty')
POLICY_LOSS_SCALING             = flags.DEFINE_float('policy_loss_scaling',
                                                    1.0,
                                                    'Scaling for the policy loss term.')
REWARD_LOSS_SCALING             = flags.DEFINE_float('reward_loss_scaling',
                                                    1.0,
                                                    'Scaling for the policy loss term.')
POLICY_LOSS_ENTROPY_REGULARIZER = flags.DEFINE_float('policy_loss_entropy_regularizer',
                                                    0.0,
                                                    'Entropy loss for the policy loss term.')
GRADIENT_NORM_CLIP              = flags.DEFINE_float('gradient_norm_clip',
                                                     0.0,
                                                    'Gradient norm clip (0 for no clip).')
DEBUG                           = flags.DEFINE_boolean('debug',
                                                    False,
                                                    'Set the code to debug')
LOGDIR                          = flags.DEFINE_string('logdir',
                                                    "fuck",
                                                    'Set the code to debug')
SERVER_ADDRESS                  = flags.DEFINE_string('server_address',
                                                    "fuck",
                                                    'Set the code to debug')
FLAGS                           = flags.FLAGS
...
def learner_config_from_file(flag_dict):
    return LearnerConfig(**flag_dict)